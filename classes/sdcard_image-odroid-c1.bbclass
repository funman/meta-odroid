...
IMAGE_TYPEDEP_odroid-c1-sdimg = "${SDIMG_ROOTFS_TYPE}"
...
IMAGE_DEPENDS_odroid-c1-sdimg = " \
			parted-native \
			mtools-native \
			dosfstools-native \
			virtual/kernel \
			${IMAGE_BOOTLOADER} \
			${@base_contains("KERNEL_IMAGETYPE", "uImage", "u-boot-hardkernel", "",d)} \
			"

# SD card image name
SDIMG = "${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.rootfs.odroid-c1-sdimg"

# Compression method to apply to SDIMG after it has been created. Supported
# compression formats are "gzip", "bzip2" or "xz". The original .odroid-c1-sdimg file
...
IMAGE_CMD_odroid-c1-sdimg () {
    ...
    # Bootloader
    dd if=${DEPLOY_DIR_IMAGE}/bl1.bin.hardkernel of=${SDIMG} conv=notrunc bs=1 count=442
    dd if=${DEPLOY_DIR_IMAGE}/bl1.bin.hardkernel of=${SDIMG} conv=notrunc bs=512 skip=1 seek=1
    dd if=${DEPLOY_DIR_IMAGE}/u-boot.bin of=${SDIMG} conv=notrunc bs=512 seek=64
